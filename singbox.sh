...
# =================================================================
# Sing-box 终极修复版 v16
# 1. 完整保留 100+ 国家地区库 (绝不阉割)
# 2. 强制清理旧版本，锁定 v1.10.1 (修复 unknown field 报错)
# =================================================================
...
# 1. 暴力清除旧版本 (绝不留情)
# ----------------------------------------------------------------
echo -e "${BLUE}>>> [1/9] 正在清除旧版本 Sing-box...${NC}"
...
if command -v sing-box >/dev/null; then
    echo -e "${RED}无法删除旧版本，请手动执行: rm -f \$(which sing-box)${NC}"
    exit 1
fi
echo -e "${GREEN}旧版本清理完毕。${NC}"

# ----------------------------------------------------------------
# 2. 下载并安装 v1.10.1 (黄金稳定版)
# ----------------------------------------------------------------
echo -e "${BLUE}>>> [2/9] 下载 Sing-box v1.10.1...${NC}"
ARCH=$(uname -m)
case $ARCH in
...
if [[ "$INSTALLED_VER" != "1.10.1" ]]; then
    echo -e "${RED}安装失败！当前版本: $INSTALLED_VER，期望: 1.10.1${NC}"; exit 1
fi
